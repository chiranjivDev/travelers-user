...
// Predefined users with proper status
export const USERS = {
  SENDER: {
    id: 'sender-1',
    name: 'John (Sender)',
...
export function SimpleChatProvider({ children }: { children: React.ReactNode }) {
  ...
  const [activePackage, setActivePackage] = useState<Package | null>(MOCK_PACKAGE)

  // Helper function to get a unique room ID for two users
  ...
  useEffect(() => {
    const handleStorageChange = (e: StorageEvent) => {
      if (e.key?.startsWith('chat_room_')) {
        ...
        // Only update if this is our current room
        if (currentUser && otherUser && e.key === `chat_room_${getRoomId(currentUser.id, otherUser.id)}`) {
  ...
  useEffect(() => {
    if (currentUser) {
      const newOtherUser = currentUser.role === 'sender' ? USERS.TRAVELER : USERS.SENDER
      setOtherUser(newOtherUser)
      
      // Load chat history for this room from localStorage
  ...
  const sendMessage = useCallback((content: string, type: Message['type'] = 'text', metadata?: any) => {
    ...
    // Update messages in both state and localStorage
    setMessages(prevMessages => {
      const newMessages = [...prevMessages, newMessage]
      localStorage.setItem(`chat_room_${roomId}`, JSON.stringify(newMessages))
      return newMessages
    })
    ...
    setTimeout(() => {
      ...
      setMessages(prev => {
        const updated = prev.map(msg => msg.id === newMessage.id ? deliveredMessage : msg)
        localStorage.setItem(`chat_room_${roomId}`, JSON.stringify(updated))
        return updated
      })
    }, 500)

    // Simulate message being read after 1 second
    setTimeout(() => {
      const readMessage = { ...newMessage, status: 'read' }
      setMessages(prev => {
        const updated = prev.map(msg => msg.id === newMessage.id ? readMessage : msg)
        localStorage.setItem(`chat_room_${roomId}`, JSON.stringify(updated))
        return updated
      })
    }, 1000)
